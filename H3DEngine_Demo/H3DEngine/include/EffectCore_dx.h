...
public:
	...
	*/
	...
	/// 设置特效进行数据同步
	virtual void SetForceSyncFlag() = 0;
	...
	// 设置是否作为遮挡体
	virtual void UseAsOccluder(bool isOccluder) = 0;
	...
	//! 勾边属性
	virtual void	SetOutlineInfo(H3DI::tOutlineInfo& oinfo) = 0;
	...
	*/
	...
	// 设置/获取是否根据此特效中的粒子效果的粒子更新AABB
	virtual void SetNeedUpdateEffAABBInSpe(bool bUpdate) = 0;
	virtual bool GetNeedUpdateEffAABBInSpe() = 0;

	//////////////////////////////////////////////////////////////////////////
	//! 根据节点名称修改节点使用的材质
	//! 目前只修改面片节点的材质，不影响其余类型的节点
	//! [参数]
	//! pNodeNameCaseSensitive : 节点名称，大小写敏感
	//! pNewShader : 材质，如果此参数为0，则表示恢复为特效最初使用的材质
	//! [返回值]
	//! 设置节点材质成功返回true
	//! 否则返回false
	//! [注意事项]
	//! 由于客户端希望自己管理外部设置的材质的生命期，因此在销毁特效之前，
	//! 必须调用此接口将修改过材质的节点使用的材质恢复为其最初使用的材质
	//! 否则必然出错
	//! 此接口仅客户端运行时有作用
	virtual bool SetTreeNodeMaterial(const char* pNodeNameCaseSensitive, H3DI::INewShader* pNewShader) = 0;
	//////////////////////////////////////////////////////////////////////////

	// 获取特效中的当前粒子数目
	// 
	virtual unsigned int getCurrentParticleCountInSpe() = 0;
