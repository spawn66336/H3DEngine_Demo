...
namespace H3DI
{
	...
	*/
	...
	{
	public:
		virtual void Export(const char* text, const char* caption)=0;
	};
	...
	public:
		...
		//获得关键帧数据 (帧数i的取值范围为[0, 总帧数-1]）
		virtual void					getposkey(float i, H3DVec3& pos)=0;
		virtual void					getrotkey(float i, H3DQuat& rot)=0 ;
	...
	public:
		...
		virtual IAnimation*				GetAnimationByName(const char* sName)=0;
	...
	*/
	...
	public:
		/* 
		 *  1、	释放INewShader的实例
			2、	释放该INewShader引用的所有贴图（如果引用计数==1的话）
			3、	该INewShader引用的材质实例资源 材质模板资源不用释放
		*/
		virtual void ReleaseImmediately()=0;
		///得到pass数量
	...
	*/
	...
	public:
		...
		virtual char*					Lock(unsigned int nType, unsigned int fmt, unsigned int stride,unsigned int eleCount)=0;
	...
	*/
	...
	enum EEntityFlag
	{
		...
		I_ENTITY_HIDE = 1 << 15, //
		I_ENTITY_FORCESYNCDATA  = 1<<16, //在PhysX线程模拟开始前强制进行线程同步
		I_ENTITY_STATIC = 1<<17,
		I_ENTITY_HIDE_UPDATE  = 1<<19, //隐藏时更新
		I_ENTITY_ADJUST_FOOT = 1<<20, //动作过度时矫正脚部高度
	};
	class IGeometry;
	...
	public:
		...
		*/
		...
		//设置预渲染标志 true表示该模型使用预渲染，false表示该模型正常渲染
		virtual void					SetFakeRender(bool b)=0;
		// 异步材质lod的同步设置接口
	...
	struct tOutlineInfo
	{
		float	m_size;		/// 勾边厚度，厚度不能无限增加，有限制，可调节
		float	m_color[4];	/// 勾边颜色
		int		m_lod;		/// 勾边lod
		tOutlineInfo(){m_size = 1.0f;memset(m_color,0,sizeof(float)*4);m_lod = 0;}
	};
	...
	public:
		...
		*/
		...
		/////! 勾边功能
		//! 勾边开关
		virtual void		SetOutlineEnable(bool b) = 0;
		virtual bool		GetOutlineEnable() = 0;
		//! 勾边属性
		virtual void		SetOutlineInfo(tOutlineInfo& oinfo) = 0;
	...
	*/
	...
	public:
		...
		*/
		...
		//根据当前动作时间更新根骨骼在当前动作中的位置
		virtual void UpdateRootPosInAction() = 0;
		//设置速率
	...
	*/
	...
	public:
		...
		*/
		...
		//获取物理网格数据
		virtual void   GetClothMeshInfo(int bptId,ClothMeshInfo& outInfo) = 0;
	...
	public:
		...
		virtual bool SetAdornmentAction(EPetAdornmentPosition adornmentposition, const char* actionName, float transitionTime = -1.0f) = 0;

		//!重置挂件
		virtual void ResetAdornment(EPetAdornmentPosition adornmentposition) = 0;
		...
		//更新挂件
		virtual void	UpdateAdornment(float fIntervalTime) = 0;


		// 卸载Actor对象的所有bodypart实例和挂件实例，以降低内存占用。
		// 想要重新正确渲染actor对象需要先调用ReloadResource()，然后重新设置所有bodypart。
		// 需要和ReloadResource接口成对调用
		virtual void UnloadResource() = 0;

		// 当一个调用了UnloadResouce()的actor对象需要重新显示时，在重新设置bodypart之前，需要调用本接口。
		// 需要和UnloadResouce接口成对调用
	...
	public:
		...
		// 参数transitionTime：从现有动作到新设动作的过渡时间，单位为秒
		// 返回值：如果成功设置了挂件中任何一个CHR的动作，返回true；如果在所有的CHR中都找不到该动作，或者对应的部位没有挂件，返回false
		virtual bool SetAdornmentAction(EActorAdornmentPosition adornmentposition, const char* actionName, float transitionTime = -1.0f) = 0;

		//!重置挂件
		virtual void ResetAdornment(EActorAdornmentPosition adornmentposition) = 0;
...
#define MAX_TERRAIN_OBJ_LAYER  16
	...
	public:
		...
		// 计算该场景中用于鼠标点选的射线
		// nMouseX:鼠标的屏幕坐标x（在view port内的相对位置xPosInViewport = xPos - viewPortX）
		// nMouseY:鼠标的屏幕坐标y（在view port内的相对位置,yPosInViewport = yPos + viewPortY + viewPortHeight - renderWindowHeight）
		// nViewPortWidth:当前viewport宽度，传0则为窗口大小
		// nViewPortHeight:当前viewport高度，传0则为窗口大小
		// pos：输出，射线起始点
		// dir: 输出，射线方向
		virtual void GetMouseRay(int nMouseX,int nMouseY, int nViewPortWidth, int nViewPortHeight, H3DVec3* pos, H3DVec3* dir) = 0;
	...
	public :
		...
		//设置某层可见,目前只能设置草和地形网格是否可见
		virtual void					SetLayerVisible(int type,bool bVisable) = 0;
		...
		virtual void					GetAlphaLayerDifferen(int iPatchIndex,int &iTextureLayer0,int &iTextureLayer1,int &iTextureLayer2,int &iTextureLayer3,int &iTextureLayer4, int alphaMapIdx) = 0;
		...
		//AlphaMap的接口
		virtual unsigned int			*GetAlphaMap(int index) = 0;
		...
		//获取一个Patch的起始X,Y
		virtual void					GetPatchPosition(int iPatchIndex, int& nPosX, int& nPosY) = 0;
		//获取一个Patch的包围盒, box是 float[6]
		virtual void					GetPatchAABB(int iPatchIndex, float* box) = 0;
		//设置一个Patch的LightMapInfo等
		virtual int						SetLightMapInfo(int iPatchIndex, const ILightMapInfo& info) = 0;
		//设置地形上每个小格的LightMap分辨率
		virtual void					SetLightMapResolution(unsigned int resolution) = 0;
		//地形patch贴图复位,把指定patch的贴图重新设置为初始状态
		virtual void					ResetPatchTexture(int iPatchIndex) = 0;

		//控制场景的显示模式：0:普通光照模式、1:纯光照模式、2:棋盘格模式、3：地形灰色显示模式
		virtual void					DisplayMode(unsigned int modeType) = 0;
		//设置棋盘格贴图，如果不设置，则调用默认的棋盘格贴图
		virtual void					SetCheckboardTexture(const char* pTexPath) = 0;
	};
...
#define ADAPTER_DESC_LEN 256
	...
	struct ResourceUsageInfo
	{
		...
		int TotalUITextureMemorySize;/// UI贴图内存总占用
		int TotalUITextureVideoSize;/// UI贴图显存总占用
	};
	...
	{
		COL_OBB_MASK = 1<<0,
		COL_DYNAMIC_MASK = 1<<1,
		COL_RACEDGE_MASK = 1<<2,
		COL_BRIDGE_MASK = 1<<3,
	};
	//各种预定义的碰撞规则
	enum ECollisionFilter
	{
		OBBRULE = COL_OBB_MASK|COL_DYNAMIC_MASK|COL_RACEDGE_MASK|COL_BRIDGE_MASK,
		DYNAMICOBBRULE = COL_OBB_MASK,
		RACERULE = COL_OBB_MASK,
		BRIDGERULE = COL_OBB_MASK,
	};
	class IRender
	{
	...
	public:
		...
		virtual bool					CreateTriangleMesh(unsigned int name,const H3DVec3* points,const unsigned int* indexs,const H3DVec3* normals,int vertexNums,int triNums,int phyId)=0;
		//!加载ASE格式的triangleMesh。内存由外界分配.需要注意的是该接口需要调用2次,1次传出大小,1次填写数据.当传入的内存指针是0时填写大小
		virtual bool					LoadTriangleMesh(const char* aseName,int& vertexNums,int& triNums,H3DVec3* points,unsigned int* indexs,H3DVec3* normals)=0;
		//!释放碰撞数据
		...
		//!射线和指定物体查询,普通射线
		virtual int						RayTestSingObject(const H3DVec3& rayOrg,const H3DVec3& rayDst,H3DVec3& collRes,unsigned int phxWorldId,unsigned int objId) = 0;
		virtual bool					RayTestCollisionWorld(int phyId,const H3DVec3& rayStart,const H3DVec3& rayEnd,H3DVec3& collPoint,H3DVec3& collNormal,int collGroup=-1,int collFilter=-1)=0;
		...
		//by lzp,重建物理世界
		virtual void RecreatePhxScene() = 0;
		virtual unsigned int GetPhxSceneShapeCount() = 0;
		...
		// 创建一个特效（lod0)
		virtual ISpecialEffect*			CreateSpe(const char* sFileName) = 0;

		// 创建某一个lod的特效
		virtual ISpecialEffect*			CreateSpe(const char* sFileName, sCreateOp& op) = 0;

		virtual IModel*					CreateModel(const char* sFileName, const H3DMat4& matLocate, IScene* pScene, int nLayer) = 0;
		//nActionUpdateType为动作更新方式（ACTION_UPDATE_X52、ACTION_UPDATE_PH）
		virtual IModel*					CreateActor(sCreateOp& op,const char* sActorName, bool bMale, unsigned int nActionUpdateType)=0;
		virtual IAvatarSkeletonModel*	CreateAvatarSkeletonModel(sCreateOp& op,const char* sActorName, bool bMale, unsigned int nActionUpdateType)=0;
		...
		*/
		...
		virtual unsigned int		CreateTextureFromFileAsyn(const char* filename, int nPriority) = 0;
		...
		//!异步加载一个动作，发起加载以后仅需通过IsActionExist判断动作是否存在
		virtual void                    LoadActionAsynManaged(const char* actName, H3DI::EActorType actorType, bool bMale, int nPriority) = 0;
		//!释放一个动作
		...
		//!得到一个动作实例
		virtual IAction*			GetAction(const char* actName, EActorType actorType, bool bMale) = 0;
		//!设置一个动作实例在ClearAllAtions时是否释放（bRelease为true表示释放）
		virtual void					SetActionRelease(const char* actName, EActorType actorType, bool bMale, bool bRelease) = 0;
		//!设置一个动作是否使用动作压缩
		virtual void					SetActionCompress(const char* actName, EActorType actorType, bool bMale, bool bCompress) = 0;
		//!读取关卡文件
		...
		*/
		...
		virtual void					DrawIndexNewEx(PRIMITIVE_TYPE nTriangleType,unsigned int nElemNum,unsigned int nVertNum,unsigned int vbOffset,unsigned int ibOffset,unsigned int nNormalOffset,unsigned int nTexcoordOffset,unsigned int nColorOffset,unsigned int nTexcoordOffset2)=0;
		///! 命令行
		...
		// 渲染模型到贴图，用于diffuse贴图烘焙
		// 使用不同diffuse贴图的网格会渲染到不同的贴图，贴图名均为原贴图名加后缀_LOD2
		// 烘焙出的贴图会跟原diffuse贴图同样大小，相同的格式
		// pModels: 模型数组指针
		// numModels: 模型数组中的模型个数
		// pLevel: 使用的场景对象，模型和灯光都应该已attach到场景中
		// bStripBody: 模型是否为素体模型，非素体模型的素体网格在烘焙时会被忽略掉
		// 返回值：烘焙是否成功
		virtual bool RenderToTexture(H3DI::IModel** pModels, int numModels, H3DI::ILevel* pLevel, bool bStripBody) = 0;
		...
		//!更新CPU蒙皮
		virtual void UpdateCpuSkin() = 0;
		//!强制同步数据
		virtual void ForceSyncData() = 0;
		//!查找一个资源是否存在（会优先查找2进制，优先从vfs中查找）
		virtual bool FindResourceExist(const char* filename)=0;
		//!设置一个错误输出对象
		virtual void SetErrorExport(IErrorExport* errorExport)=0;
	};
	...
	public:
		...
		//////////////////////////////////////////////////////////////////////////
		//编辑器接口
		virtual void SetVisibleDistEnable(bool isEnable) = 0;;
	...
	//!打印引擎存于内存的临时Log。目前是打印到引擎日志文件当中。
	//!此函数只有在IRender* CreateRenderer();调用之后；void DeleteRenderer()调用之前使用。
	//!返回值，bool :是否成功打印。返回false说明内部全局数据已经被破坏
	//!注意，如果是在dump时，调用此函数。最好在做完其他更重要的工作之后，最后再调用此函数。
	//!      因为dump时，引擎内部一些数据可能已经被破坏，调用此函数也许会有问题。
	bool PrintTempLog();
