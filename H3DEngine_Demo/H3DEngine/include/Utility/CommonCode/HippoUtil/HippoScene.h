...
public:
	...
	virtual H3DI::IModel*				CreateDml(const char* fn,int matlod=0);
	virtual ISpecialEffect*				CreateSpe(const char* fn,int matlod=0);
	virtual H3DI::INewShader*			CreateShader(const char* fn,const char* matn);
	virtual H3DI::ISkeletonModel*		CreateChr(const char* fn,int matlod=0);
	...
	unsigned int CreateChrAysn(const char* fn, int matlod);
...
public:
	...
	HippoScene*			GetSceneByIdx(unsigned int idx);
