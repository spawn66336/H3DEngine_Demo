...
//#define H3D_ENABLE_NEDMALLOC_LOG	/// @@@ LH： 开启Nedmalloc的日志模式
#ifdef H3D_ENABLE_NEDMALLOC_LOG
...
#define NEDMALLOC_STACKBACKTRACEDEPTH 8
#endif
...
NEDMALLOCEXTSPEC void enableStackWalkerNed(bool enable)
{
	s_recordMem = enable;
}
#endif
...
{ 
	...
	if (first_new)
	{
		...
		if(handle)
		{
			...
			addPtr = (voidPtrVoid)GetProcAddress(handle,"AddPtr");
...
#endif
	void* addr = nedpmalloc((nedpool *) 0, size);
#ifdef USE_STACKWALKER
	if (addPtr&&s_recordMem)
	{
		addPtr(addr);
	}
#endif
