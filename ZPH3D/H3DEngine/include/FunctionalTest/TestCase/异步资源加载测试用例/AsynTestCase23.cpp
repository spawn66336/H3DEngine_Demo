...
Task0完成后不要设置给人物，cancel掉，等到引擎加载队列数目为0，执行下一步。
clearmatlib
给actor0以同步的方式设置lod1
渲染5s观察效果
释放所有人物
...
class AsynTestCase23 : public HippoTestCase_RefCompare
{
public:
public:
	AsynTestCase23(const char* casename):HippoTestCase_RefCompare(casename){}
	...
	enum ExeState
	{
		...
		//以异步的方式给actor1切换到lod1（task0）
		ASYN_SET_LOD,
		//Task0完成后不要设置给人物，cancel掉，
		AFTER_LOADED_CANCEL_TASK0,
		//等到引擎加载队列数目为0，执行下一步。
		WAIT_QUEUE_IS_ZERO,
...
public:
	...
	virtual bool ManualRunExec()
	{
		...
		case AFTER_LOADED_CANCEL_TASK0://Task0完成后不要设置给人物，cancel掉，
				{ 
		...
		case WAIT_QUEUE_IS_ZERO://等到引擎加载队列数目为0，执行下一步。
				...
				break;			
		case SET_LOD_ACTOR1://clearmatlib   给actor0以同步的方式设置lod1
...
ADD_TESTCASE("ASYN_LOAD_TEST",AsynTestCase23);